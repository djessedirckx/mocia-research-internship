...
def random_search(matchnet_config: MatchNetConfig, window_lengths: List):
    ...
    for window_length in window_lengths:
        print(f'Executing randomsearch for window_size={window_length}\n')

        # Create configuration for the desired sliding window length
        search_config = RandomSearchConfig((window_length, 35), (window_length, 35))
        data_creator = DataCreator(window_length=window_length, prediction_horizon=matchnet_config.pred_horizon)
        ...
        # Store hyperparameters of best result
        best_models[window_length] = tuner.get_best_hyperparameters()
...
if __name__ == '__main__':
    ...
    window_lengths = list(range(3, 11))
